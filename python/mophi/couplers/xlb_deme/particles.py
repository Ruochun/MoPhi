"""GPU-native DEME particle state made available to XLB-side Warp code."""
...
class XLBDEMEParticleExchange:
    """Expose DEME particle-owner positions and velocities to XLB kernels.

    This transports state only. It deliberately does not prescribe a fluid
    interpolation, drag law, or XLB-to-DEME force model.
    """
    ...
    def initialize(self, deme_solver, first_owner_id: int, owner_count: int, device):
        ...
        if self.initialized:
            raise RuntimeError("XLBDEMEParticleExchange is already initialized.")
        ...
        if not device.is_cuda:
            raise ValueError("XLB--DEME particle exchange requires a CUDA Warp device.")
        for method in (
        ...
        if device.ordinal not in tuple(int(value) for value in deme_solver.GetGPUDeviceIDs()):
            raise ValueError("DEME and XLB must share the selected CUDA device.")
    ...
    def read_deme_particle_state(self):
        ...
        if not self.initialized:
            raise RuntimeError("XLBDEMEParticleExchange must be initialized before use.")
