# MoPhi visualization architecture

This document explains and justifies the public interface implemented by the
pure-Python visualizer backends in `python/mophi/visualizers/`, especially
`opengl_visualizer.py` and `omniverse_visualizer.py`. Those package files are
the canonical implementations; there is no second source copy under `src/`.
...
### 5. Backends share a stable rendering interface

Both visualizers expose the same frame and generic-primitive methods so that
demos can switch backends by changing a single constructor call:
...
as no-ops (e.g. `set_camera` on the USD backend). The USD backend additionally
provides `set_mesh_shapes` and `set_cube_shapes` for baking Newton visual-shape
descriptors into an exported stage; these are setup extensions rather than
part of the shared per-frame rendering contract.
...
The Omniverse backend also implements these export-specific setup methods:

| Method | Signature | Notes |
|--------|-----------|-------|
| `set_mesh_shapes` | `(mesh_descriptors)` | Bake static `UsdGeom.Mesh` children under animated body xforms |
| `set_cube_shapes` | `(cube_descriptors)` | Bake static `UsdGeom.Cube` children under animated body xforms |
...
**`set_mesh_shapes` / `set_cube_shapes`**: Register Newton visual geometry that
cannot be inferred from `state.body_q`. Descriptors supplied before the first
state sample are applied once the animated body hierarchy exists.
...
1. Create `python/mophi/visualizers/<name>_visualizer.py`.
...
4. Update this document to describe and justify the new backend.
